...
int main(int argc, char *argv[]) {
    ...
    */
    ...
    for (int t = 1; t < T; t++) { // à chaque tour
        ...
        l_particules[i].oldax = l_particules[i].ax;
        l_particules[i].olday = l_particules[i].ay; 
        axi1 = ayi1 = 0;
            for (int j = 0; j < N; j++) { // pour chaque couple i-j
                if ( i == j ) {continue;}
                F_ij[i][j].x = force(l_particules[i].x,l_particules[i].y,l_particules[j].x,l_particules[j].y) * ( l_particules[j].x - l_particules[i].x );
                F_ij[i][j].y = force(l_particules[i].x,l_particules[i].y,l_particules[j].x,l_particules[j].y) * ( l_particules[j].y - l_particules[i].y );
                axi1 += F_ij[i][j].x;
                ayi1 += F_ij[i][j].y;    
            }
        ...
        for(int i = 0;i<N;i++){ 
            ...
            l_particules[i].vx += (0.5*dt)*(l_particules[i].oldax + l_particules[i].ax);
            l_particules[i].vy += (0.5*dt)*(l_particules[i].olday + l_particules[i].ay);   
        }
